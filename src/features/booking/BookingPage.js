...
const BookingPage = () => {
    ...
    // Simulate receiving triage score from state or query (default 0)
    const triageScore = location.state?.score || 0;
    ...
    return (
        <div className="max-w-7xl mx-auto px-4 py-12 lg:py-20">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between mb-12 gap-6">
                ...
                <div className="flex items-center space-x-3 bg-white p-2 border border-slate-200 rounded-2xl shadow-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {doctors.map((doc, idx) => (
                    <motion.div
                        ...
                        whileHover={{ y: -8 }}
                        className={`card-premium group relative ${selectedDoctor === doc.id ? 'ring-2 ring-primary border-primary' : ''}`}
            ...
            <AnimatePresence>
                {(selectedDoctor && selectedSlot) && (
                    <motion.div
                        ...
                        animate={{ opacity: 1, y: 0 }}
                        exit={{ opacity: 0, y: 50 }}
                        className="fixed bottom-10 left-1/2 -translate-x-1/2 w-[90%] max-w-xl z-40"
                    >
                        <div className="bg-slate-900 rounded-3xl p-6 shadow-2xl flex items-center justify-between border border-white/10 backdrop-blur-xl">
                            <div className="flex items-center space-x-4">
                            ...
                            <button
                                className="px-8 py-3 bg-primary text-white font-black rounded-xl hover:bg-teal-800 transition-all text-sm uppercase tracking-widest shadow-lg shadow-primary/20"
                                onClick={() => {
